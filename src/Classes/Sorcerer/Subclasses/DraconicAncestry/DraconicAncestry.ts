...
import { SorcererSubclassParams } from "../SorcererSubclass";
...
export class DraconicAncestry {
  ...
  static draconicAncestry6(pc: PlayerCharacter, params: LevelingParams) {
    pc.pcHelper.addFeatures({
      ...DraconicAncestry.getFeature("6", "ELEMENTAL AFFINITY"),
      choices: [
        DraconicBloodline[
          (params.subclassParams as SorcererSubclassParams).draconicAncestry
        ]["Damage Type"],
      ],
    });
    pc.baseStats.HpMax.bonus.value++;
  ...
  static draconicAncestry18(pc: PlayerCharacter, params: LevelingParams) {
    pc.pcHelper.addFeatures(
      DraconicAncestry.getFeature("18", "DRACONIC PRESENCE")
    );
    pc.baseStats.HpMax.bonus.value++;
  }

  static draconicResilienceHpBonus(
    pc: PlayerCharacter,
